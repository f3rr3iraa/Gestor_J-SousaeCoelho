...
async function initFormSupabase() {
  if (!window.supabaseClient) {
    ...
    console.error(
      "❌ Supabase SDK não foi carregado. Verifica o script no index.html"
    );
    return;
  }
  ...
  itemForm.addEventListener("submit", async (e) => {
    ...
    if (fotoFile && fotoFile.name) {
      ...
      try {
        ...
        if (publicError) {
          ...
          showMessage(
            "Erro ao obter URL da imagem: " + publicError.message,
            "danger"
          );
          return;
        }
        ...
        showMessage(
          "Erro ao carregar imagem: " + (err.message || err),
          "danger"
        );
        return;
      }
    ...
    if (error) {
      ...
      showMessage(
        "Erro ao gravar no banco de dados: " + error.message,
        "danger"
      );
      return;
    }
